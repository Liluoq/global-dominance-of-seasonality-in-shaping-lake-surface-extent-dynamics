...
def process_date_range(basin_id, start_date, input_gsw_folder_pattern, gsw_image_file_name_pattern, output_file_path_pattern):
    """Process GSW images for a given basin and date range.

    This function mosaics GSW (Global Surface Water) image tiles for a specific basin and time period.
    It creates monthly mosaics from individual tiles using the provided file patterns and paths.

    Args:
        basin_id (int): ID of the basin to process
        start_date (datetime): Start date for the mosaic
        input_gsw_folder_pattern (str): Pattern for input GSW folder path with {basin_id} placeholder
        gsw_image_file_name_pattern (str): Pattern for GSW image filenames with {basin_id}, {start_date}, {end_date} placeholders
        output_file_path_pattern (str): Pattern for output mosaic filepath with {basin_id}, {start_date}, {end_date} placeholders
