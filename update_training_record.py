...
def update_training_record(model_name, trained_status=None, training_set_size=None, 
    ...
    """Update or create a training record for a model in a CSV file.
    
    Parameters:
        model_name (str): Name of the model to update/create record for
        trained_status (bool, optional): Whether model has been trained
        training_set_size (int, optional): Size of training dataset
        evaluation_set_size (int, optional): Size of evaluation dataset  
        test_set_size (int, optional): Size of test dataset
        final_decision (str, optional): Final decision on model performance
        training_acc (float, optional): Training accuracy
        eval_acc (float, optional): Evaluation accuracy
        test_acc (float, optional): Test accuracy
        training_IoU (float, optional): Training IoU score
        eval_IoU (float, optional): Evaluation IoU score 
        test_IoU (float, optional): Test IoU score
        keep_ratio (float, optional): Keep ratio for training
        batch_processed (bool, optional): Whether batch processing has been completed
        file_path (str, optional): Path to CSV file. Defaults to 'training_records.csv'
        
    Returns:
        None
        
    The function either updates an existing record or creates a new one in the CSV file.
    Only provided parameters will be updated for existing records.
    If the CSV file doesn't exist, it will be created with the required columns.
    """
