...
def draw_pie_with_different_radius(
    ...
    """
    Draw a pie chart with different radius for each slice.

    Args:
        datas (list): List of numerical values for each pie slice
        colors (list): List of colors corresponding to each slice
        ax (matplotlib.axes.Axes, optional): Polar axis to draw on. If None, creates new figure. Defaults to None.

    Returns:
        matplotlib.axes.Axes: The axis with the drawn pie chart

    Notes:
        - Slices are drawn with radius proportional to their value
        - Percentages are displayed in the center of each slice
        - Slices are drawn clockwise starting from 0 degrees
        - Requires a polar projection axis
    """
...
def category_percentage_pie_plots(
    ...
    """
    Create pie charts showing percentages for categorical and continuous variables.

    Args:
        to_plot_df (pandas.DataFrame): DataFrame containing the data to plot
        continuous_column_names (list): Names of continuous columns to bin and plot
        continuous_group_bins_list (list): List of bin edges for each continuous column
        continuous_group_labels_list (list): List of labels for binned continuous groups
        continuous_group_colors_list (list): List of colors for continuous group pie slices
        discrete_column_names (list): Names of discrete/categorical columns to plot
        discrete_group_values_list (list): List of possible values for each discrete column
        discrete_group_labels_list (list): List of labels for discrete groups
        discrete_group_colors_list (list): List of colors for discrete group pie slices
        ax_list (list, optional): List of matplotlib axes to plot on. If None, creates new figure. Defaults to None.
        save_path (str, optional): Path to save the figure. Defaults to None.
        pies_different_radius (bool, optional): Whether to draw pies with varying radius. Defaults to False.

    Returns:
        tuple: Figure and axes objects if ax_list is None, otherwise None

    Raises:
        ValueError: If both continuous_column_names and discrete_column_names are None
    """
...
def bar_plot_of_different_groups(
    ...
    """
    Create a bar plot showing statistics for different groups of data.

    Args:
        to_plot_df (pandas.DataFrame): DataFrame containing the data to plot
        to_plot_column_names (list): List of column names to plot
        to_plot_column_colors (list): List of colors for each column
        to_plot_column_labels (list): List of labels for each column
        agg_to_bar_stat_types (list): List of statistical aggregation types for each column
        to_group_by_column_name (str): Column name to group the data by
        group_bins (list, optional): Bin edges for continuous grouping. Defaults to None.
        group_values (list, optional): Discrete values for grouping. Defaults to None.
        group_labels (list, optional): Labels for the groups. Defaults to None.
        bar_width (float, optional): Width of the bars. Defaults to 0.35.
        xticklabel_rotation (float, optional): Rotation angle of x-axis labels. Defaults to 0.
        x_label (str, optional): Label for x-axis. Defaults to 'Category'.
        y_label (str, optional): Label for y-axis. Defaults to 'Changes'.
        ax_title (str, optional): Title for the plot. Defaults to 'Changes of different groups'.
    ...
    Raises:
        ValueError: If neither group_bins nor group_values is provided
        ValueError: If both group_bins and group_values are provided
    """
...
def percentage_per_category_bar_plot_emsemble(
    ...
    """Create a bar plot showing percentages of trend categories across different groups.

    Args:
        lake_lse_df (pd.DataFrame): DataFrame containing lake data
        trend_column_name (str): Column name containing trend values
        significant_trend_column_name (str): Column name containing significance of trends
        category_column_name (str): Column name to group by
        category_bins (array-like, optional): Bins to group category values. Defaults to None.
        category_values (array-like, optional): Discrete values to group by. Defaults to None.
        category_labels (list, optional): Labels for the categories. Defaults to None.
        bar_width (float, optional): Width of the bars. Defaults to 0.9.
        bar_span_color_2d_list (list of lists, optional): Colors for each bar segment. Defaults to None.
        xticklabel_color_list (list, optional): Colors for x-axis tick labels. Defaults to None.
        ax (matplotlib.axes.Axes, optional): Axes to plot on. Defaults to None.
    ...
    Raises:
        ValueError: If neither category_bins nor category_values is provided
        ValueError: If both category_bins and category_values are provided
    """
...
def box_plot_of_different_groups(
    ...
    """Create box plots showing distribution of values across different groups.
    
    Args:
        to_plot_df (pd.DataFrame): DataFrame containing the data to plot
        to_plot_column_name (str): Name of column containing values to plot
        to_group_by_column_name (str): Name of column to group data by
        group_bins (array-like): Bin edges for grouping values
        group_labels (list): Labels for each group
        group_colors_box (list or ScalarMappable, optional): Colors for box fills. If ScalarMappable, colors boxes by median values.
        group_colors_median (list, optional): Colors for median lines
        box_width (float, optional): Width of boxes. Defaults to 0.35.
        vmin (float, optional): Minimum value for color mapping
        vmax (float, optional): Maximum value for color mapping
        use_bootstrap (bool, optional): Whether to use bootstrapping. Defaults to False.
        show_fliers (bool, optional): Whether to show outlier points. Defaults to False.
        bootstrap_statistic (callable, optional): Statistic to bootstrap. Defaults to np.median.
        ax (matplotlib.axes.Axes, optional): Axes to plot on. Creates new figure if None.
        xticklabel_rotation (float, optional): Rotation angle for x-tick labels. Defaults to 45.
        show_plot (bool, optional): Whether to display the plot. Defaults to True.
    ...
    if use_bootstrap:
        ...
        bp = ax.boxplot(data, positions=np.arange(len(group_labels)), widths=box_width, 
                showfliers=show_fliers, patch_artist=patch_artist)
...
def histogram_of_different_groups(
    ...
    """Create histograms for different groups in the data.

    Parameters
    ----------
    to_plot_df : pandas.DataFrame
        DataFrame containing the data to plot
    to_plot_column_name : str
        Name of the column containing the values to plot in histograms
    to_group_by_column_name : str
        Name of the column used to group the data
    group_bins : array-like
        Bin edges for grouping the data
    group_labels : list
        Labels for each group
    hist_bins : int or array-like
        Number of bins or bin edges for histograms
    hist_colors : dict
        Dictionary mapping group labels to colors
    hist_alpha : float, optional
        Transparency of histograms, by default 0.5
    hist_density : bool, optional
        If True, plot density instead of counts, by default True
    hist_type : str, optional
        Type of histogram ('bar', 'barstacked', 'step', 'stepfilled'), by default 'step'
    ax : matplotlib.axes.Axes, optional
        Axes object to plot on. If None, creates new figure, by default None
...
def create_2d_cmap_discrete_four_corner(
    ...
    """Creates a discrete 2D colormap with interpolated colors between four corner colors and an optional center color.

    Parameters
    ----------
    lower_left_corner_hex_color : str
        Hex color code for the lower left corner
    lower_right_corner_hex_color : str 
        Hex color code for the lower right corner
    upper_right_corner_hex_color : str
        Hex color code for the upper right corner
    upper_left_corner_hex_color : str
        Hex color code for the upper left corner
    num_intervals_vertical : int
        Number of vertical intervals for discretization
    num_intervals_horizontal : int
        Number of horizontal intervals for discretization
    center_hex_color : str, optional
        Hex color code for the center point, by default None
    center_weight_adjust : float, optional
        Adjusts the weight of the center color interpolation, by default 1.0
    return_matrix : bool, optional
        If True, returns the RGBA color matrix instead of colormap, by default False

    Returns
    -------
    matplotlib.colors.ListedColormap or numpy.ndarray
        If return_matrix is False, returns a discrete colormap
        If return_matrix is True, returns the RGBA color matrix
    """
...
def create_2d_colorbar_vertical_interpolation(
    ...
    """Creates a 2D colorbar with vertical interpolation between two sets of colors.

    Parameters
    ----------
    lower_hex_colors : list of str
        List of hex color codes for the lower colors
    upper_hex_colors : list of str 
        List of hex color codes for the upper colors
    num_intervals_vertical : int
        Number of vertical intervals for interpolation
    return_matrix : bool, optional
        If True, returns the RGBA color matrix instead of colormap, by default False
...
def create_2d_colorbar_from_rgba_matrix(
    ...
    """Creates a 2D colorbar from an RGBA color matrix.

    Parameters
    ----------
    rgba_matrix : numpy.ndarray
        Matrix of RGBA colors with shape (n_vertical, n_horizontal, 4)
    return_matrix : bool, optional
        If True, returns the RGBA color matrix instead of colormap, by default False
...
def draw_2d_colorbar(
    ...
    """Draws a 2D colorbar with custom tick positions and labels.

    Parameters
    ----------
    cmap : matplotlib.colors.Colormap
        The colormap to use for the 2D colorbar
    num_intervals_horizontal : int
        Number of intervals along the horizontal axis
    num_intervals_vertical : int 
        Number of intervals along the vertical axis
    xtick_positions : array-like, optional
        Positions of ticks along x-axis. If None, no ticks are shown.
    xtick_labels : array-like, optional
        Labels for x-axis ticks. If None, no labels are shown.
    ytick_positions : array-like, optional
        Positions of ticks along y-axis. If None, no ticks are shown.
    ytick_labels : array-like, optional
        Labels for y-axis ticks. If None, no labels are shown.
    xlabel : str, optional
        Label for x-axis. If None, no label is shown.
    ylabel : str, optional
        Label for y-axis. If None, no label is shown.
    ax : matplotlib.axes.Axes, optional
        Axes on which to draw the colorbar. If None, creates new figure and axes.

    Returns
    -------
    matplotlib.axes.Axes
        The axes containing the drawn colorbar
    """
...
def _find_grid_cell(x_intervals, y_intervals, point, origional='lower left'):
    """Find the grid cell indices for a point in a 2D grid.

    Parameters
    ----------
    x_intervals : array-like
        Array of x-axis interval boundaries
    y_intervals : array-like 
        Array of y-axis interval boundaries
    point : tuple
        (x,y) coordinates of the point
    origional : str, optional
        Origin position of the grid. Currently only 'lower left' is supported.
        Default is 'lower left'.

    Returns
    -------
    tuple
        (x_index, y_index) indices of the grid cell containing the point

    Raises
    ------
    ValueError
        If origional is not 'lower left'
    """
    x, y = point
...
def generate_color_index(
    ...
    """
    Generate a color index for a row based on its x and y values falling within intervals.

    Parameters
    ----------
    row : pandas.Series
        Row of data containing x and y values
    x_column_name : str
    ...
    -------
    int
        Color index corresponding to the grid cell containing the point
    """
...
def gdf_plotting_using_2d_colobar(
    ...
    """
    Plot a GeoDataFrame using a 2D colormap based on two columns of data.

    Args:
        to_plot_gdf (geopandas.GeoDataFrame): GeoDataFrame to plot
        x_column_name (str): Name of column to use for x-axis values
        y_column_name (str): Name of column to use for y-axis values 
        x_intervals (list): List defining intervals for x-axis binning
        y_intervals (list): List defining intervals for y-axis binning
        cmap_2d (matplotlib.colors.LinearSegmentedColormap, optional): Custom 2D colormap. Defaults to None.
        lower_left_corner_hex_color (str, optional): Hex color for lower left corner. Defaults to None.
        lower_right_corner_hex_color (str, optional): Hex color for lower right corner. Defaults to None.
        upper_right_corner_hex_color (str, optional): Hex color for upper right corner. Defaults to None.
        upper_left_corner_hex_color (str, optional): Hex color for upper left corner. Defaults to None.
        temporary_color_column_name (str, optional): Name for temporary color index column. Defaults to 'temporary_color_column'.
        center_hex_color (str, optional): Hex color for center point. Defaults to None.
        center_weight_adjust (float, optional): Weight adjustment for center color. Defaults to 1.0.
        ax (matplotlib.axes.Axes, optional): Axes to plot on. Defaults to None.
        fix_for_meridian (bool, optional): Whether to handle antimeridian crossing. Defaults to True.
        projection (cartopy.crs, optional): Map projection. Defaults to PlateCarree.
        extent (list, optional): Map extent [xmin, ymin, xmax, ymax]. Defaults to None.
        gridlines (bool, optional): Whether to show gridlines. Defaults to True.
        title (str, optional): Plot title. Defaults to None.
        set_global (bool, optional): Whether to set global extent. Defaults to True.
        return_ax (bool, optional): Whether to return axes object. Defaults to True.
        return_cmap (bool, optional): Whether to return colormap. Defaults to True.
        plot_cmap (bool, optional): Whether to plot colormap. Defaults to True.
        cmap_xtick_positions (list, optional): X-tick positions for colormap. Defaults to None.
        cmap_xtick_labels (list, optional): X-tick labels for colormap. Defaults to None.
        cmap_ytick_positions (list, optional): Y-tick positions for colormap. Defaults to None.
        cmap_ytick_labels (list, optional): Y-tick labels for colormap. Defaults to None.
        cmap_xlabel (str, optional): X-axis label for colormap. Defaults to None.
        cmap_ylabel (str, optional): Y-axis label for colormap. Defaults to None.
        add_pie_percentage (bool, optional): Whether to add pie charts. Defaults to False.
        pie_proportional_radius (bool, optional): Whether pie chart radius is proportional. Defaults to False.
        additional_pie_data (pandas.DataFrame, optional): Additional data for pie charts. Defaults to None.
        additional_pie_colors (list, optional): Colors for pie chart segments. Defaults to None.
    ...
    Returns:
        tuple: Tuple containing figure, axes and colormap if return_ax and return_cmap are True

    Raises:
        ValueError: If required color parameters are missing when cmap_2d not provided
    """
...
def violin_plots_of_different_groups(
    ...
    """Create violin plots comparing distributions across different groups.

    Args:
        to_plot_df (pd.DataFrame): DataFrame containing the data to plot
        to_plot_column_name (str): Name of column containing values to plot
        to_group_by_column_name (str): Name of column to group by
        group_bins (array-like, optional): Bin edges for grouping numerical data. Defaults to None.
        group_labels (list, optional): Labels for the groups. Defaults to None.
        group_values (list, optional): Discrete values to group by. Defaults to None.
        group_colors (list or matplotlib colormap): Colors for each violin plot. Defaults to None.
        show_means (bool, optional): Whether to show mean markers. Defaults to True.
        mean_color (str, optional): Color of mean markers. Defaults to 'white'.
        show_medians (bool, optional): Whether to show median markers. Defaults to True.
        median_color (str, optional): Color of median markers. Defaults to 'white'.
        ax (matplotlib.axes.Axes, optional): Axes to plot on. If None, creates new figure. Defaults to None.
        title (str, optional): Plot title. Defaults to 'Violin plots of different groups'.
        add_whiskers (bool, optional): Whether to add whiskers showing data range. Defaults to True.
        violin_positions (array-like, optional): Custom positions for violin plots. Defaults to None.
        violin_label_positions (array-like, optional): Custom positions for x-axis labels. Defaults to None.

    Returns:
        matplotlib.axes.Axes: The axes containing the violin plots
