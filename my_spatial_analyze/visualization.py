...
def split_antimeridian(geometry):
    """Split a geometry that crosses the antimeridian (180/-180 degrees longitude).
    
    This function splits geometries that cross the antimeridian into multiple geometries
    and adjusts their coordinates to ensure proper display on a map.

    Args:
        geometry (shapely.geometry): The input geometry to be split

    Returns:
        list: A list of split geometries. If the input geometry doesn't cross the 
              antimeridian, returns a list containing only the original geometry.
              
    Notes:
        - Handles geometries that cross the 180/-180 degree meridian line
        - Translates coordinates as needed to maintain proper spatial relationships
        - Returns empty geometries unchanged
        - Adjusts geometries that extend beyond -180/180 degrees
    """
...
def handle_antimeridian(gdf):
    """Handle geometries that cross the antimeridian in a GeoDataFrame.
    
    This function processes a GeoDataFrame containing geometries that may cross the 
    antimeridian (180/-180 degrees longitude) by splitting them appropriately.

    Args:
        gdf (geopandas.GeoDataFrame): The input GeoDataFrame containing geometries
            that may cross the antimeridian.

    Returns:
        geopandas.GeoDataFrame: A new GeoDataFrame with split geometries. Geometries 
            that crossed the antimeridian are split into multiple parts, with all other
            attributes duplicated for the split geometries.
            
    Notes:
        - Preserves all columns and attributes from the original GeoDataFrame
        - Handles multiple geometry types (Polygon, MultiPolygon, etc.)
        - May increase the number of rows if geometries are split
        - Maintains proper spatial relationships across the antimeridian
    """
    exploded_geoms = []
...
def plot_grid(grid_gdf, ax=None, projection=ccrs.PlateCarree(), 
    ...
    if color_column:
        ...
        if not use_discrete_color:
            ...
            norm = BoundaryNorm(discrete_bins, ncolors=cmap.N, extend=cbar_extend)
        ...
        if draw_colorbar:
            fig.colorbar(sm, ax=ax, extend=cbar_extend)
    else:
...
def plot_grid_with_panels(
    ...
    """Plot a grid GeoDataFrame with additional panels showing statistics along axes.

    Args:
        grid_gdf (GeoDataFrame): GeoDataFrame containing grid cells to plot
        ax (matplotlib.axes.Axes, optional): Main axes to plot on. If None, creates new figure. Defaults to None.
        inset_axes_layout (dict, optional): Layout specifications for inset axes. If None, uses default layout. Defaults to None.
        main_plot_kwargs (dict): Keyword arguments passed to plot_grid() for main plot
        panel_columns (list, optional): Columns to calculate panel statistics from. If None, uses main plot color column. Defaults to None.
        panel_stat_lists (list of lists): Lists of statistics to calculate for each panel column. Defaults to [['sum']].
        panel_steps (float): Step size for binning along axes. Defaults to 1.0.
        panel_legend_lists (list of lists): Legend labels for each panel statistic. Defaults to [['Sum']].
        panel_color_lists (list of lists): Colors for each panel line. Defaults to [['black']].
        additional_panel_plot_kwargs (dict): Additional keyword arguments for panel plots. Defaults to {}.
        additional_colorbar_kwargs (dict): Additional keyword arguments for colorbar. Defaults to {}.
        save_path (str, optional): Path to save figure. If None, does not save. Defaults to None.

    Returns:
        matplotlib.axes.Axes: Main plot axes object
    """
