...
def glakes_update_hydrolakes_all():
    """Update HydroLAKES geometries using GLAKES data.

    This function reads GLAKES shapefiles for different continents and updates the geometries
    of corresponding lakes in the HydroLAKES dataset. The process involves:
    1. Reading and concatenating GLAKES shapefiles from different continents
    2. Performing a spatial join between GLAKES and HydroLAKES
    3. Dissolving GLAKES geometries by HydroLAKES ID
    4. Updating HydroLAKES geometries with corresponding GLAKES geometries
    5. Filtering to keep only unique geometries with largest lake area
    6. Saving the updated HydroLAKES shapefile

    The updated shapefile is saved in a new directory with '_updated_using_GLAKES' suffix.
    """
...
def hydrolakes_divide_by_basin():
    """Divides HydroLAKES shapefile into separate files by basin.
    
    This function reads the HydroLAKES polygon and point shapefiles along with a basin 
    shapefile (HydroBASINS), and divides the HydroLAKES data into separate files for 
    each basin. The process involves:

    1. Reading input shapefiles:
       - HydroLAKES polygons
       - HydroLAKES points 
       - HydroBASINS basins

    2. Checking and aligning coordinate reference systems (CRS)
    
    3. For each basin:
       - Finding HydroLAKES points within the basin
       - Extracting corresponding lake polygons
       - Saving basin-specific lakes to separate files

    The output files are saved in a 'per_basin' subdirectory with filenames following 
    the pattern 'hylak_original_unbuffered_{hybas_id}.shp'
    """
...
def glakes_update_hydrolakes_per_basin():
    """Updates HydroLAKES lake polygons using GLAKES data.
    
    This function processes HydroLAKES data basin by basin, updating lake polygons
    using more recent GLAKES data. For each basin:
    1. Loads the HydroLAKES polygons for that basin
    2. Loads all GLAKES polygons 
    3. For each HydroLAKES lake, finds overlapping GLAKES polygons and updates geometry
    4. Saves updated HydroLAKES polygons to new files
    
    The input HydroLAKES files should be in 'per_basin' subdirectory with filenames
    following pattern 'hylak_buffered_{hybas_id}.shp'
    
    The output updated files are saved in a new directory with filenames following
    pattern 'hylak_unbuffered_{hybas_id}_updated.shp'
