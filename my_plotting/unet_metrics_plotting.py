...
def plot_unet_metrics(metrics_gdf, projection, axes_layout, axes_kws, metrics, extent=None, carto_plot_kwargs=None, sns_plot_kwargs=None, box_column_names=None, plot_type='box_plot', save_path=None):
    """Plot U-Net metrics either as box/violin plots or on a map.

    This function creates visualizations of U-Net metrics using either statistical plots
    (box plots or violin plots) for comparing distributions, or geographical plots showing
    metric values on a map.

    Args:
        metrics_gdf (GeoDataFrame): GeoDataFrame containing metrics and geometries
        projection (cartopy.crs): Cartopy projection to use for map plots
        axes_layout (dict): Dictionary defining the subplot layout
        axes_kws (dict): Dictionary of keyword arguments for each subplot axis
        metrics (list): List of metrics to plot. Each element can be a single metric name
            or a list of metric names to compare
        extent (list, optional): Map extent as [min_lon, max_lon, min_lat, max_lat]
        carto_plot_kwargs (dict, optional): Keyword arguments for cartopy map plots
        sns_plot_kwargs (dict, optional): Keyword arguments for seaborn statistical plots
        box_column_names (list, optional): Names for box/violin plot categories
        plot_type (str, optional): Type of statistical plot - 'box_plot' or 'violin_plot'
    ...
    Raises:
        ValueError: If plot_type is not 'box_plot' or 'violin_plot'
    """
    #sns.set_style("white")
