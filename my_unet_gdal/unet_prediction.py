...
def predict_and_save_to_tfrecord_async(
        ...
        """
        Asynchronously perform predictions using a model on a dataset and save the predictions to a TFRecord file.

        Args:
            model (tf.keras.Model): The model to use for predictions.
            tfrecord_path (str): Path to the TFRecord file.
            input_band_names (list): List of band feature names in the TFRecord file.
            input_band_scaling_factors (list): List of scaling factors for input bands.
            input_dtype (tf.dtypes.DType): Data type of the parsed window tensor.
            batch_size (int): Batch size for the dataset.
            output_band_name (str): Name of the output band in the TFRecord file.
            output_file (str): Output file path.
            output_dtype (tf.dtypes.DType): Data type for the output predictions.
            verbose (int, optional): Verbosity level. Defaults to 0.
            compression_type (str, optional): Compression type for TFRecord. Defaults to 'GZIP'.
            num_processes (int, optional): Number of processes for parallel processing. Defaults to 1.
            max_queue_size_per_process (int, optional): Maximum queue size per process. Defaults to 4.

        Returns:
            Future: A Future object representing the asynchronous save operation.
        """
        if verbose != 0:
            print(f"Performing predictions and saving results from input: {tfrecord_path}")
