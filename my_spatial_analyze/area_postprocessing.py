...
def generate_monthly_ice_mask_for_area(
    ...
    """Generate monthly ice mask ratios for lake area time series.

    This function processes ice height data from a LakeEnsemblR NetCDF file to generate monthly ratios 
    of days with ice cover. It can handle cases where the area time series extends beyond the model data
    by using a 5-year average of recent months to fill in future values.

    Args:
        nc_file_path (str): Path to the LakeEnsemblR NetCDF file containing ice height data
        model_start_date (str): Start date of the model data in date_fmt format
        model_end_date (str): End date of the model data in date_fmt format  
        area_start_date (str): Start date of the area time series in date_fmt format
        area_end_date (str): End date of the area time series in date_fmt format
        date_fmt (str, optional): Date format string. Defaults to '%Y-%m-%d'
        model_selection_strategy (str, optional): Strategy for selecting ice height value from multiple models.
            Must be one of ['min', 'median', 'max']. Defaults to 'min', using 'median' is equivalent to using Simstrat
        missing_value (float, optional): Value used to represent missing data. Defaults to 1e20

    Returns:
        list: Monthly ratios of days with ice cover for each month in the area time series period.
            Values range from 0 (no ice) to 1 (ice every day).

    Raises:
        ValueError: If model_selection_strategy is not one of ['min', 'median', 'max']
    """
...
def get_ice_existence_from_mylake_txt(
    ...
    """Get ice existence from MyLake txt file.

    This function reads a MyLake txt file and extracts the ice existence data.
    """
...
def generate_monthly_ice_mask_for_area_from_mylake_txt(
    ...
    """Generate monthly ice mask ratios for lake area time series from MyLake txt file.

    This function processes ice existence data from a MyLake txt file to generate monthly ratios 
    of days with ice cover. It can handle cases where the area time series extends beyond the model data
    by using a 5-year average of recent months to fill in future values.
    """
    model_start_date = datetime.strptime(model_start_date, date_fmt)
...
def mask_area_when_frozen(
    ...
    """Mask lake areas during frozen periods and optionally detect outliers.
    
    Args:
        area_concatenated_df (pd.DataFrame): DataFrame containing lake area time series data
        area_columns (list): List of column names containing area values
        lake_id_column (str): Name of column containing lake IDs
        ice_ratio_lake_id_array (np.ndarray): Array of lake IDs corresponding to ice ratio data
        ice_ratio_array (np.ndarray): Array of ice ratio values
        ice_ratio_threshold (float, optional): Threshold above which to consider lake frozen. Defaults to 0.3
        mask_column_prefix (str, optional): Prefix for masked area column names. Defaults to None
        use_parallel (bool, optional): Whether to use parallel processing. Defaults to True
        n_jobs (int, optional): Number of parallel jobs. -1 means use all cores. Defaults to -1
        outlier_detection (bool, optional): Whether to detect outliers in non-frozen periods. Defaults to False
        lof_n_neighbors (int, optional): Number of neighbors for LOF outlier detection. Defaults to 12
        lake_area_column_name (str, optional): Name of lake area column for outlier detection. Required if outlier_detection=True
        
    Returns:
        pd.DataFrame: DataFrame with added masking columns and outlier detection results if enabled
        
    Raises:
        ValueError: If lake_area_column_name not provided when outlier_detection=True
    """
...
def fix_unmasked_lakes(
    ...
    """Fix unmasked lakes by applying ice masking based on nearest masked lake.

    This function processes lakes that were not initially masked for ice coverage by finding their
    nearest masked lake neighbor and applying that lake's ice masking pattern.

    Args:
        area_partially_masked_gdf (gpd.GeoDataFrame): GeoDataFrame containing lake areas with partial ice masking
        area_columns (list): List of column names containing area measurements
        lake_id_column (str): Name of column containing lake IDs
        lake_geom_column (str): Name of column containing lake geometries
        area_mask_flag_column (str): Name of column indicating if a lake is masked
        ice_ratio_lake_id_array (np.ndarray): Array of lake IDs corresponding to ice ratios
        ice_ratio_array (np.ndarray): Array of ice coverage ratios
        ice_ratio_threshold (float, optional): Threshold for ice ratio masking. Defaults to 0.3.
        mask_column_prefix (str, optional): Prefix for masked area columns. Defaults to None.
        use_parallel (bool, optional): Whether to use parallel processing. Defaults to True.
        outlier_detection (bool, optional): Whether to perform outlier detection. Defaults to False.
        lof_n_neighbors (int, optional): Number of neighbors for LOF outlier detection. Defaults to 12.
        lake_area_column_name (str, optional): Name of column containing lake areas. Required if outlier_detection is True.

    Returns:
        gpd.GeoDataFrame: Updated GeoDataFrame with ice masking applied to previously unmasked lakes
    
    Raises:
        ValueError: If outlier_detection is True but lake_area_column_name is not provided
    """
...
def cloud_cover_ratio_calculation(
    ...
    """Calculate cloud cover ratios for lake areas.

    This function calculates the cloud cover ratios for each lake in the cloud_cover_area_df based on the area of the corresponding lake in the lake_shp_gdf.
    """
...
def mask_area_when_cloud_contaminated(
    ...
    """Mask lake areas when cloud cover ratio is above a threshold.
    
    This function masks lake areas when the cloud cover ratio is above a specified threshold. For those below the threshold, it will also perform outlier detection.
    """
