# This script is used to generate grid data from lake-wise data
...
def calculate_statistics_along_direction(
    ...
    """Calculate statistics for a geographic data frame along a specified axis.
    
    Args:
        gdf (GeoDataFrame): Input geodataframe containing geometry and data column
        column (str): Name of the column to calculate statistics on
        axis (str, optional): Direction to bin data - either 'longitude' or 'latitude'. Defaults to 'longitude'.
        step (float, optional): Size of bins in degrees. Defaults to 1.0.
        stat_type (str, optional): Type of statistic to calculate. Must be one of:
            'mean', 'median', 'sum', 'count', 'sum_positive', 'sum_negative', 
            'mean_positive', 'mean_negative'. Defaults to 'mean'.
    
    Returns:
        DataFrame: Contains bin intervals, calculated statistic values, and bin centers
        
    Raises:
        ValueError: If axis is not 'longitude' or 'latitude'
        ValueError: If stat_type is not one of the valid options
    """
...
def grid_time_series_analysis(
    ...
    """Performs time series analysis on a GeoDataFrame containing grid data.

    Args:
        grid_gdf (GeoDataFrame): Input GeoDataFrame containing grid geometries and time series data
        time_series_columns (list): List of column names containing the time series data
        type_of_analysis (str): Type of time series analysis to perform
        output_column_name (str): Name of the output column to store analysis results
        unit_scale (float, optional): Scale factor to apply to values. Defaults to 1.0
        mk_test_significance_level (float, optional): Significance level for Mann-Kendall test. Defaults to 0.05

    Returns:
        GeoDataFrame: Copy of input GeoDataFrame with additional columns containing analysis results
    """
...
if __name__ == '__main__':
    TEST = False
    GENERATE_FULL_GRID_AREA = False
    TIME_SERIES_ANALYZE = False
    GENERATE_GRID_FOR_RELATIVE_TO_TOTAL_AREA_REVISION = True
    if TEST:
        # Sample GeoDataFrame
        data = {
            'geometry': [
    ...
    if GENERATE_FULL_GRID_AREA:
        ...
        if selection_type == 'all':
            grid_gdf_save_paths = [
                '/WORK/Data/global_lake_area/area_csvs/grids/pkl/grid_all_coarse.pkl',
                '/WORK/Data/global_lake_area/area_csvs/grids/csv/grid_all_coarse.csv'
            ]
    ...
    if GENERATE_GRID_FOR_RELATIVE_TO_TOTAL_AREA_REVISION:
        analyzed_column_agg_type = [
            'sum', 'sum', 'sum',
            'median', 'median', 'median',
            ('sum', 'pos_count', 'neg_count'), 'median', ('sum', 'pos_count', 'neg_count'),
        ...
        config = load_config_module('LAKE_WISE_AREA_POSTPROCESSING_CONFIG.py')
        area_columns = config.AREA_COLUMNS
        lake_columns_oi = [config.LAKE_ID_COLUMN] + area_columns + ['centroid', 'geometry', 'Lake_type', 'total_variation_relative_to_total_area'] + aggregate_column_names + [item for sublist in bootstrap_ks_test_column_pairs for item in sublist]
        lake_full_pkl_path = '/WORK/Data/global_lake_area/area_csvs/lakes/pkl/lakes_all_with_all_additional_attributes_revision_with_missing_data_and_unbuffered_boundary_area_and_total_variation_relative_to_total_area.pkl'
        lake_full_gdf = pd.read_pickle(lake_full_pkl_path)[lake_columns_oi]
        ...
        if selection_type == 'all':
            grid_gdf_save_paths = [
                '/WORK/Data/global_lake_area/area_csvs/grids/pkl/grid_all_medium_relative_to_total_area.pkl',
                '/WORK/Data/global_lake_area/area_csvs/grids/csv/grid_all_medium_relative_to_total_area.csv'
            ]
        elif selection_type == 'natural':
            lake_full_gdf.drop(lake_full_gdf[lake_full_gdf['Lake_type'] != 1].index, inplace=True)
            grid_gdf_save_paths = [
