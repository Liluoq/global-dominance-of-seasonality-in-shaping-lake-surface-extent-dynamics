...
def get_raster_subset(raster_path, x_min, x_max, y_min, y_max):
    """
    Extract a subset of a raster image based on projected coordinates.
    
    Args:
        raster_path (str): Path to the input raster file
        x_min (float): Minimum x-coordinate (longitude) of the subset
        x_max (float): Maximum x-coordinate (longitude) of the subset
        y_min (float): Minimum y-coordinate (latitude) of the subset
        y_max (float): Maximum y-coordinate (latitude) of the subset
    
    Returns:
        numpy.ndarray: A 2D array containing the extracted raster subset
        
    Raises:
        RuntimeError: If the raster file cannot be opened
        SystemExit: If there is an error opening the raster file
    """
...
def plot_raster_section(raster_section):
    """
    Plot a raster section using a binary colormap.
    
    Args:
        raster_section (numpy.ndarray): A 2D array containing the raster data to plot,
            where values should be binary (0 for non-water, 1 for water)
    
    Returns:
        None: Displays the plot using matplotlib
        
    Notes:
        - Uses a yellow/blue colormap where yellow represents non-water (0) 
          and blue represents water (1)
        - Includes a colorbar with labels for water/non-water
        - Plot size is fixed at 6x6 inches
    """
