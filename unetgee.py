...
def read_samples(training_pattern, eval_pattern, local_sample_folder, input_bands, input_band_scaling_factors:list, response, kernel_size, batch_size, only_sizes=False, compression_type=None):
    """Read and parse TFRecord samples for training and evaluation.
    
    Args:
        training_pattern (str): Pattern to match training TFRecord files
        eval_pattern (str): Pattern to match evaluation TFRecord files 
        local_sample_folder (str): Local folder containing TFRecord files
        input_bands (list): List of input band names
        input_band_scaling_factors (list): List of scaling factors for input bands
        response (list): List of response/target band names
        kernel_size (int): Size of the input kernel/window
        batch_size (int): Batch size for training
        only_sizes (bool, optional): If True, only return dataset sizes. Defaults to False.
        compression_type (str, optional): Compression type of TFRecords. Defaults to None.

    Returns:
        If only_sizes=True:
            tuple: (training_size, eval_size) - Number of training and evaluation samples
        If only_sizes=False:  
            tuple: (training_dataset, eval_dataset, training_size, eval_size) - TensorFlow datasets and sizes
    """
    ...
    def parse_tfrecord(example_proto):
        """Parse a single TFRecord example into a dictionary of features.
        
        Args:
            example_proto (bytes): Serialized TFRecord example
        
        Returns:
            dict: Parsed TFRecord data
        """
        return tf.io.parse_single_example(example_proto, features_dict)
    
    def to_tuple(inputs):
        """Convert the parsed TFRecord data into a tuple of input and response tensors.
        
        Args:
            inputs (dict): Parsed TFRecord data
        
        Returns:
            tuple: (scaled_inputs, response) - Scaled input bands and response bands
        """
    ...
    def predicate(inputs):
        """Filter out samples based on the predicate.
    ...
    def get_dataset_size():
        """Get the size of the training and evaluation datasets.
    ...
    def get_dataset(pattern):
        """Get the TensorFlow dataset for a given pattern.
        
        Args:
            pattern (str): Pattern to match TFRecord files
    ...
    def get_training_dataset(shuffle_buffer_size):
        """Get the TensorFlow dataset for training.
    ...
    def get_eval_dataset():
        """Get the TensorFlow dataset for evaluation.
        
        Returns:
            tf.data.Dataset: TensorFlow dataset for evaluation
        """
...
def read_test_samples(test_pattern, local_sample_folder, input_bands, input_band_scaling_factors, response, kernel_size, compression_type=None):
    """Read and parse TFRecord samples for testing.
    
    Args:
        test_pattern (str): Pattern to match test TFRecord files
        local_sample_folder (str): Local folder containing TFRecord files
        input_bands (list): List of input band names
        input_band_scaling_factors (list): List of scaling factors for input bands
        response (list): List of response/target band names
    ...
    Returns:
        tf.data.Dataset: TensorFlow dataset for testing
    """
    kernel_shape = [kernel_size, kernel_size]
    features = input_bands + response
    columns = [
    ...
    def get_test_dataset():
        """Get the TensorFlow dataset for testing.
...
def draw_unet_train_figure(history, save_path):
    """Draw a figure for UNet training history.
...
def unet_train(training_pattern, eval_pattern, local_sample_folder, input_bands, input_band_scaling_factors:list, response,
    ...
    """
    Train a U-Net model for water classification.
    
    Parameters
    ----------
    training_pattern : str
        Pattern to match training data files
    eval_pattern : str 
        Pattern to match evaluation data files
    local_sample_folder : str
        Path to folder containing sample data
    input_bands : list
        List of input band names
    input_band_scaling_factors : list
        List of scaling factors for input bands
    response : list
    ...
    save_models_and_figures_folder : str
        Path to save trained models and figures
    keep_ratio : float, optional
        Ratio of samples to keep, by default 1
        
    Returns
    -------
    None
        Saves trained model and training history plots to disk
    """
...
def unet_train_per_feature(collection_size, local_sample_folder, save_models_and_figures_folder, 
    ...
    """
    Train a U-Net model for water classification for each feature in a collection.
    ...
    save_models_and_figures_folder : str
        Path to save trained models and figures
    training_pattern : str, optional
        Pattern to match training data files, by default 'training'
    eval_pattern : str, optional
        Pattern to match evaluation data files, by default 'eval'
    input_bands : list, optional
        List of input band names, by default ['AWEI_MODIS']
    response : list, optional
        List of response variable names, by default ['Water_GSW']
    kernel_size : int, optional
        Size of input kernel, by default 128
    epochs : int, optional
        Number of training epochs, by default 150
    batch_size : int, optional
        Batch size for training, by default 32
    start_feature_index : int, optional
        Index of the first feature to train, by default 0
    end_feature_index : int, optional
        Index of the last feature to train, by default 1e10
    """
...
def unet_evaluate_on_test(model_path, test_pattern, test_sample_folder, input_bands=['AWEI_MODIS'], response=['Water_GSW'], kernel_size=128):
    """
    Evaluate a trained U-Net model on test data.
    ...
    test_pattern : str
        Pattern to match test data files
    test_sample_folder : str
        Path to folder containing test samples
    input_bands : list, optional
        List of input band names, by default ['AWEI_MODIS']
    response : list, optional
        List of response variable names, by default ['Water_GSW']
...
def image_generation(start_time, end_time, kernel_size, mode, input_bands:list, input_bands_type='int16', response=['Water_GSW'], custom_projection_wkt=None, custom_scale=500, return_mask=False, test_display=False, gsw_max_start_date='2021-12-01', gsw_max_end_date='2022-01-01'):
    """Generate MODIS median composites, joint neighborhood composites, and GSW monthly history.
    Parameters:
        start_time (str): Start date for the image generation
        end_time (str): End date for the image generation
        kernel_size (int): Size of the kernel for joint neighborhood composites
        mode (str): Mode of the image generation
        input_bands (list): List of input bands
        input_bands_type (str): Data type of input bands
        response (list): List of response variables
        custom_projection_wkt (str): Custom projection WKT
        custom_scale (int): Custom scale (meters per pixel)
        return_mask (bool): Whether to return the mask
        test_display (bool): Whether to display the test image
        gsw_max_start_date (str): Maximum start date for GSW monthly history
        gsw_max_end_date (str): Maximum end date for GSW monthly history
    Returns:
        ee.Image: MODIS median composites, joint neighborhood composites, or GSW monthly history
        geemap.Map: Map for testing display
    """
...
def unet_sample_generation_time_period(region, start_time, end_time, generate_samples, train_size, eval_size, test_size, shard_size, 
    ...
    """Generate training, evaluation, and test samples for a U-Net model.
    
    Parameters:
        region (ee.Geometry): Region to generate samples
        start_time (str): Start date for the sample generation
        end_time (str): End date for the sample generation
        generate_samples (bool): Whether to generate samples
        train_size (int): Size of training samples
        eval_size (int): Size of evaluation samples
        test_size (int): Size of test samples
        shard_size (int): Size of each shard
        training_base (str): Base name for training samples
        eval_base (str): Base name for evaluation samples
        test_base (str): Base name for test samples
        starting_seed (int): Seed for random sampling
        export_to (str): Destination for exported samples
        input_bands (list): List of input bands
        response (list): List of response variables
        train_shard_num (int): Number of training shards
        eval_shard_num (int): Number of evaluation shards
        test_shard_num (int): Number of test shards
        custom_projection_wkt (str): Custom projection WKT
        custom_scale (int): Custom scale (meters per pixel)
        mode (str): Mode of the image generation
        local_task_id_folder (str): Folder for task IDs
        local_task_id_base (str): Base name for task IDs
        drive_folder (str): Folder for Google Drive export
        bucket (str): Bucket for Google Cloud Storage export
        gcs_folder (str): Folder for Google Cloud Storage export
        kernel_size (int): Size of the kernel for joint neighborhood composites
        scale (int): Scale for sampling
    
    Returns:
        list: List of task IDs for training, evaluation, and test samples
    """
