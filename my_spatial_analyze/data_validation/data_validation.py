...
def plot_using_datashader(ax, x, y, add_colorbar=True):
    """Plot data points using datashader for density visualization.
    ...
    add_colorbar : bool, optional
        Whether to add a colorbar showing density scale (default True)
        
    Returns
    -------
    None
        The plot is added to the provided axes object
    """
...
def plot_using_scipy_gaussian_kde(ax, x, y, add_colorbar=True):
    """Plot data points using scipy's gaussian KDE for density visualization.
...
def compare_with_glolakes_lake_wise(
    ...
    """Deprecated. Use compare_with_gsw instead."""
    if not my_lakes_area_pkl_path.endswith('.pkl'):
        raise ValueError('The file extension of the my_lakes_area_pkl_path is not recognized. The file extension should be .pkl.')
    glolakes_df = pd.read_csv(glolakes_csv_path)
    my_lakes_gdf = pd.read_pickle(my_lakes_area_pkl_path)
...
def compare_with_xingdongLi_2019_lake_wise(
    ...
    """Generate plots that compare the lake area from my results with the lake area from Xingdong Li's 2019 paper.
    """
    ...
    if num_lakes % num_cols != 0:
        num_rows += 1
    fig, axs = plt.subplots(num_rows, num_cols, figsize=(5*num_cols, 5*num_rows), layout='compressed')
    ax_list = axs.flatten()
    all_cc = []
    for lake_name, ax in zip(lake_names, ax_list):
        ...
        ax.set_title(lake_name.replace('_', ' '))
