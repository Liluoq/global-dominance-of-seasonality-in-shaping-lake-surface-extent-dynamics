...
def attach_geometry_to_concatenated_areas(
    ...
    """Attach geometry from a shapefile to concatenated area data.

    This function joins geometry data from a shapefile to area measurements stored in a CSV file
    or DataFrame based on a common lake ID field. It can optionally convert polygons to centroids
    and reproject to a different CRS.

    Args:
        concatenated_area_csv (str or pd.DataFrame): Path to CSV file containing area measurements
            or pandas DataFrame with the measurements
        lake_shp_path (str): Path to shapefile containing lake geometries
        lake_id_field_in_csv (str): Name of lake ID column in the CSV/DataFrame
        lake_id_field_in_shp (str): Name of lake ID column in the shapefile
        common_id_name (str): Name to use for the joined ID column
        cast_to_centroid (bool, optional): Whether to convert polygons to centroids. Defaults to False.
        output_pkl_path (str, optional): Path to save output as pickle file. Defaults to None.
        output_crs (str, optional): CRS to reproject geometries to. Defaults to None.
        verbose (int, optional): Verbosity level (0=quiet, 1=basic info, 2=detailed). Defaults to 0.

    Returns:
        gpd.GeoDataFrame: GeoDataFrame containing the area measurements joined with geometries

    Raises:
        ValueError: If concatenated_area_csv is neither a string path nor DataFrame
    """
...
def generate_grid_from_geometry_added_concatenated_areas(
    ...
    """Generate a grid from concatenated area data with attached geometry.

    This function generates a grid from concatenated area data that has been joined with lake geometries.
    It can perform a K-S test for two-sided and one-sided tests, and aggregate data based on specified operations.
    """
    if grid_extent:
...
def time_series_analysis_on_df(
    ...
    This function performs various statistical analyses on a DataFrame containing time series data.
    It supports multiple types of analysis, including mean, standard deviation, mean first difference,
