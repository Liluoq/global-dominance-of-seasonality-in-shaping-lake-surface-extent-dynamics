...
def lake_wise_lse_analyze_and_plot(
    ...
    """Analyze and plot lake-wise LSE (Lake Surface Extent) trends and seasonality.
    
    This function performs time series analysis on lake surface extent data and creates 
    visualization plots. It is deprecated in favor of lake_wise_lse_analyze_and_plot_v2.

    Args:
        lake_lse_df (pd.DataFrame): DataFrame containing lake surface extent time series data
        lake_id_column_name (str): Name of column containing lake IDs
        area_columns (list): List of column names containing area measurements
        period (int, optional): Period for seasonal decomposition. Defaults to 12.
        reshape_period (int, optional): Period for reshaping data. Defaults to 12.
        do_analyze (bool, optional): Whether to perform time series analysis. Defaults to True.
        do_plotting (bool, optional): Whether to create plots. Defaults to True.
        unit_scale (float, optional): Scale factor for units. Defaults to 1e-6.
        stl_trend_column_name (str, optional): Name for trend column. Defaults to 'stl_trend'.
        stl_season_column_name (str, optional): Name for seasonality column. Defaults to 'stl_season_max_minus_min'.
        mean_area_column_name (str, optional): Name for mean area column. Defaults to 'mean_area'.
        lake_size_bins (list, optional): Bin edges for lake size categories. Defaults to None.
        lake_size_labels (list, optional): Labels for lake size categories. Defaults to None.
        save_path (str, optional): Path to save output plots. Defaults to None.

    Returns:
        pd.DataFrame: DataFrame with added trend and seasonality analysis columns
        
    Raises:
        ValueError: If required columns are missing when do_analyze is False
    """
...
def calculate_statistics(
    ...
    """Calculate statistics for lake-wise data.

    This function calculates statistics for lake-wise data based on the specified analysis column.
    It supports two types of statistics: MK significant trend percentage and linear trend percentage.
    """
...
def basin_distribution_plot(
    ...
    """Plot basin distribution.

    This function plots the distribution of basins on a map.
    """
    if ax is None:
...
def basin_wise_statistics_plot(
    ...
    """Plot basin-wise statistics.

    This function plots the statistics for each basin on a map.
    """
    if ax is None:
        fig, ax = plt.subplots(subplot_kw={'projection': projection}, figsize=(15, 7.5))
...
def basin_time_series_plot(
    ...
    This function plots the time series for each basin.
    """
...
def categorical_percentage_pie_plot(
    ...
    """Plot percentage pie charts for each category.

    This function plots percentage pie charts for each category.
    """
...
def bar_plot_of_all_lse_changes(
    ...
    This function plots bar charts for each category.
    """
